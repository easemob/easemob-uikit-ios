...
public func convert8khzWaveToAMR(wave data : Data) -> Data? {
...
public func convert16khzWaveToAmr(wave data : Data) -> Data? {
...
fileprivate func waveHeaderData(sampleRate : UInt32, dataSize : UInt32) -> Data {
    ...
    let ckFmt = ChunkFormat(header: fmtHeader, audioFormat: 1, numChannels: 1, sampleRate: sampleRate, byteRate: sampleRate*2, blockAlign: 2, bitsPerSample: 16)
...
fileprivate func convertAmrToWave(isWB : Bool, data : Data) -> Data? {
    ...
    repeat {
        if amrData.count >= Int(frameSize) {
            let frameRange = 0..<Int(frameSize) 
            let frameData = amrData[frameRange]
