...
@IBDesignable
public class RippleAnimationView: UIView {
    ...
    @IBInspectable public var diskColor: UIColor = .systemBlue {
        didSet {
    ...
    @objc private func animateCentralDisk() {
        ...
        let alphaAnimation = Animation.opacity(from: 0.6, to: 0.0)
    ...
    @objc private func animateCircles() {
        for index in 0 ..< self.circlesLayer.count {
            let colorAnimation = Animation.color(from: circleOffColor.cgColor, to: self.circleOnColor.cgColor)
