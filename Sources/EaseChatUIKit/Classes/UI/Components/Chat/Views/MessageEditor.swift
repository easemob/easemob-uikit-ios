...
@objcMembers open class MessageEditor: UIView {
    ...
    private let statusImage = UIImage(chatNamed: "edit_bar_status")
...
extension MessageEditor: ThemeSwitchProtocol {
    public func switchTheme(style: ThemeStyle) {
        ...
        self.done.image(UIImage(chatNamed: "uncheck")?.withTintColor(style == .dark ? UIColor.theme.neutralColor4:UIColor.theme.neutralColor7, renderingMode: .alwaysOriginal), .disabled).image(UIImage(chatNamed: "check")?.withTintColor(style == .dark ? UIColor.theme.primaryDarkColor:UIColor.theme.primaryLightColor), .normal)
